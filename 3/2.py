...
def point_in_line(point: Point, line: Line):
    if point.x == line.startpoint.x:
        if line.min_y <= point.y <= line.max_y:
            return abs(point.y - line.startpoint.y)
    if point.y == line.startpoint.y:
        if line.min_x <= point.x <= line.max_x:
            return abs(point.x - line.startpoint.x)
    return -1

    

def steps_to_point(wire, intersectionpoint):
    steps = 0
    for i, l in enumerate(wire):
        pil = point_in_line(intersectionpoint, l)
        if pil != -1:
            return steps + pil
        else:
            steps += l.length
...
def sum_of_steps(wirelines, intersectionpoint):
    steps_1 = steps_to_point(wirelines[0], intersectionpoint)
    steps_2 = steps_to_point(wirelines[1], intersectionpoint)
    ...
    for l in wirelines[0]:
        for l2 in wirelines[1]:
            ...
            for intersection in i:
                intersectiondict[intersection] = sum_of_steps(wirelines, intersection)
